...
def get_mood_gradient(mood):
    ...
    return gradients.get(mood.lower(), gradients["neutral"])
...
def render_lottie(url, height=300, key=None):
    ...
    The optional `key` is accepted here, but **not** passed into .html().
    """
    lottie_html = f"""
...
def apply_mood_specific_styling(mood):
    ...
    cfg = mood_styles.get(mood.lower(), mood_styles["neutral"])
    st.markdown(f"""
    <style>
      :root {{--mood-accent:{cfg['accent_color']};--animation-speed:{cfg['animation_speed']};}}
      .stApp {{ animation-duration: var(--animation-speed) !important; }}
      .stSelectbox > div > div {{ border-color: var(--mood-accent) !important; }}
      .stTextInput > div > div > input:focus {{
        border-color: var(--mood-accent) !important;
        box-shadow:0 0 0 .2rem {cfg['accent_color']}40 !important;
      }}
      .mood-indicator::before {{
        content:"{cfg['emoji']}";font-size:2em;position:fixed;top:20px;right:20px;opacity:.7;z-index:1000;
      }}
    </style>
